...
bool Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds,
    ...
    for (auto &cond : curr_conds) {
        if (cond.is_rhs_val && cond.lhs_col.tab_name.compare(tab_name) == 0) unique_cols.insert(cond.lhs_col.col_name);
    }
    index_col_names = std::vector<std::string>(unique_cols.begin(), unique_cols.end());
