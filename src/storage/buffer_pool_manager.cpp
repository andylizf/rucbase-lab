...
bool BufferPoolManager::find_victim_page(frame_id_t* frame_id) {
    ...
    if (victim_page->is_dirty_ && victim_page->id_.fd != INVALID_PAGE_ID) {
        disk_manager_->write_page(victim_page->id_.fd, victim_page->id_.page_no, 
                                victim_page->data_, PAGE_SIZE);
    }

    if (victim_page->id_.fd != INVALID_PAGE_ID) {
...
void BufferPoolManager::update_page(Page *page, PageId new_page_id, frame_id_t new_frame_id) {
    if (page->is_dirty_ && page->id_.fd != INVALID_PAGE_ID) {
        disk_manager_->write_page(page->id_.fd, page->id_.page_no, page->data_, PAGE_SIZE);
    }

    if (page->id_.fd != INVALID_PAGE_ID) {
        page_table_.erase(page->id_);
    }
    if (new_page_id.fd != INVALID_PAGE_ID) {
...
bool BufferPoolManager::delete_page(PageId page_id) {
    ...
    update_page(page, {INVALID_PAGE_ID, INVALID_PAGE_ID}, frame_id);
